...
// AlertTickerType represents the type classification of an AlertTicker (PRIMARY vs SECONDARY).
type AlertTickerType string

const (
	AlertTickerTypePrimary   AlertTickerType = "PRIMARY"
	AlertTickerTypeSecondary AlertTickerType = "SECONDARY"
)
...
type AlertTicker struct {
	...
	Type         AlertTickerType `gorm:"column:type;not null;default:SECONDARY" json:"type" binding:"required,oneof=PRIMARY SECONDARY"`
	Ticker       *Ticker         `gorm:"foreignKey:TickerID;references:ID" json:"-"`
	PriceAlerts  []PriceAlert    `gorm:"foreignKey:AlertTickerID;references:ID;constraint:OnDelete:CASCADE" json:"-"`
...
type AlertTickerQuery struct {
	...
	Type     AlertTickerType `form:"type" binding:"omitempty,oneof=PRIMARY SECONDARY"`
}
