...
type Note struct {
	...
	JournalID string `gorm:"column:journal_id;not null;index:idx_note_journal_status,priority:1" json:"journal_id"`
	//nolint:lll // Long validation tag required for oneof
	Status    string    `gorm:"column:status;not null;index:idx_note_journal_status,priority:2" json:"status" binding:"required,oneof=SET RUNNING DROPPED TAKEN REJECTED SUCCESS FAIL MISSED JUST_LOSS BROKEN"`
