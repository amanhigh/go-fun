...
var _ = Describe("TickerManager", func() {
	...
	Context("GetPrice", func() {
		...
		Context("with split adjustments", func() {
			var (
				ticker    = "SPLIT_ADJ"
				stockData tax.StockData
				queryDate time.Time
	...
	Context("GetDailyPrices", func() {
		...
		Context("with split adjustments", func() {
			...
			Context("with previous year-end carry-over pricing", func() {
				BeforeEach(func() {
					stockData = tax.StockData{
						Prices: map[string]float64{
							"2023-12-31": 100.00, // Previous year-end price
				...
				It("should include previous year-end price adjusted by future split factor", func() {
					Expect(pricesErr).ToNot(HaveOccurred())
					// Should have 3 prices: previous year-end + 2 current year prices
					Expect(prices).To(HaveLen(3))
					// Previous year-end price should be adjusted by future split factor: 100 * 2/1 = 200
					Expect(prices["2023-12-31"]).To(Equal(200.00))
					// Current year pre-split price should also be adjusted: 100 * 2/1 = 200
					Expect(prices["2024-01-10"]).To(Equal(200.00))
					// On split date price should be unchanged
					Expect(prices["2024-03-01"]).To(Equal(50.00))
				})
			})
		})
	...
	Context("GetSplits", func() {
		...
		Context("with splits matching date range", func() {
			BeforeEach(func() {
				from = time.Date(2024, 2, 1, 0, 0, 0, 0, time.UTC)
				to = time.Date(2024, 4, 1, 0, 0, 0, 0, time.UTC)
			})

			It("should return only splits within the requested range", func() {
