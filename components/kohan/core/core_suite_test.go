...
var _ = BeforeSuite(func() {
	...
	journalRepo := repository.NewJournalRepository(baseRepository)
	journalMgr := manager.NewJournalManager(journalRepo)
	journalHandler := handler.NewJournalHandler(journalMgr)
	imageHandler := handler.NewImageHandler(manager.NewImageManager(journalMgr, repository.NewImageRepository(baseRepository)))
	noteHandler := handler.NewNoteHandler(manager.NewNoteManager(journalMgr, repository.NewNoteRepository(baseRepository)))
	tagHandler := handler.NewTagHandler(manager.NewTagManager(journalMgr, repository.NewTagRepository(baseRepository)))
	tickerRepo := repository.NewTickerRepository(baseRepository)
	tickerMgr := manager.NewBarkatTickerManager(tickerRepo)
	tickerHandler := handler.NewTickerHandler(tickerMgr)
	alertTickerRepo := repository.NewAlertTickerRepository(baseRepository)
	...
	priceAlertRepo := repository.NewPriceAlertRepository(baseRepository)
	priceAlertMgr := manager.NewPriceAlertManager(priceAlertRepo)
	priceAlertHandler := handler.NewPriceAlertHandler(priceAlertMgr)
	auditRepo := repository.NewAuditRepository(baseRepository)
