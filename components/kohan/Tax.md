...
1. **Layer 1 (Input):** User-provided brokerage exports and parsed transaction data
...
└── Output/                                  [LAYER 3: Generated]
    ...
    │
    ├── YearEndBalance/
    │   ├── accounts_2023.csv                Computed year-end portfolio (2023)
...
**Input/Parsed/**
- **`trades.csv`** - Buy/sell transactions extracted from vested.xlsx
- **`dividends.csv`** - Dividend records extracted from vested.xlsx
- **`interest.csv`** - Interest records extracted from vested.xlsx
- Generated by `tax vested parse` command and stored here for subsequent computations
...
#### Output/Parsed/ (from `tax vested parse`)
- **`trades.csv`** - Buy/sell transactions extracted from vested.xlsx
- **`dividends.csv`** - Dividend records extracted from vested.xlsx
- **`interest.csv`** - Interest records extracted from vested.xlsx
...
- **`accounts_YYYY.csv`** - Portfolio snapshot at December 31st, YYYY
  ...
  - Represents all holdings and their valuations
  - **Becomes Layer 1 input for next year's computation**
  - Example: `accounts_2024.csv` becomes the starting point for computing 2025 gains
...
```
vested.xlsx (Layer 1: User Input)
    ↓
    ├─→ [tax vested parse]
    │   ↓
    │   Output/Parsed/
    ...
    │   └── interest.csv
    │
    ├─→ accounts_2023.csv (Layer 3 from prior year)
    ├─→ Data/Tickers/*.json (Layer 2)
    ├─→ Data/Reference/sbi_rates.csv (Layer 2)
    │
    └─→ [tax compute]
        ↓
        Output/YearEndBalance/
        ├── accounts_2024.csv ← becomes Layer 1 input for 2025 computation
        │
        Output/Computed/
        └── gains.csv
        │
...
1. System starts with empty portfolio (no `accounts_2021.csv`)
2. Processes `vested.xlsx` to extract 2022 transactions
3. Computes `accounts_2022.csv` as the first year-end balance
4. This becomes the starting point for 2023 computation

### Year-to-Year Continuation

For subsequent years:

1. Prior year's `accounts_YYYY.csv` (Layer 3 output) is manually moved to `Input/` (Layer 1)
2. New `vested.xlsx` for current year is placed in `Input/`
3. System processes and generates new `accounts_YYYY+1.csv` in `Output/YearEndBalance/`
4. Cycle repeats annually
