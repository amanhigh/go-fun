...
The system uses a **three-layer immutable data architecture**:

1. **Layer 1 (Input):** User-provided prior year-end portfolio snapshots
2. **Layer 2 (Data):** External reference data (ticker prices, exchange rates)
3. **Layer 3 (Output):** System-generated results and computations

### Directory Structure (Runtime: FARoot = ~/Downloads/FACompute/)

```
FARoot (~/Downloads/FACompute/)
├── Input/                                    [LAYER 1: User Input]
│   ├── Brokerage/
│   │   └── vested.xlsx                      DriveWealth brokerage export
│   │
│   └── Parsed/
│       ├── trades.csv                       From `tax vested parse`
│       ├── dividends.csv                    From `tax vested parse`
│       ├── interest.csv                     From `tax vested parse`
│       └── accounts_2023.csv                Prior year-end portfolio (2023)
│
├── Data/                                    [LAYER 2: External Data]
│   ├── Tickers/
│   │   ├── AAPL.json                        Historical daily prices from Yahoo Finance
│   │   ├── MSFT.json
│   │   ├── VWO.json
│   │   ├── IEF.json
│   │   └── NVDA.json                        (auto-downloaded on first use)
│   │
│   └── Reference/
│       └── sbi_rates.csv                    SBI TT Buy rates (USD-INR)
│
└── Output/                                  [LAYER 3: Generated]
    ├── Computed/
    │   └── gains.csv                        From FIFO algorithm
    │
    ├── YearEndBalance/
    │   ├── accounts_2024.csv                Computed year-end portfolio (2024)
    │   └── accounts_2025.csv                Computed year-end portfolio (2025)
    │
    └── Reports/
        └── tax_summary_2024.xlsx            Final ITR report with INR conversions
```

### Test Data Structure (components/kohan/testdata/tax/)

Test data is completely flat (single layer) for simplicity. It contains reference files used during testing:

```
testdata/tax/
├── AAPL.json                                Reference ticker prices
├── MSFT.json
├── VWO.json
├── IEF.json
├── sbi_rates.csv                           Reference exchange rates (SBI TT Buy)
├── vested.xlsx                             Reference brokerage export
├── accounts_2022.csv                       Reference portfolio snapshot
├── trades.csv                              Reference parsed trades
├── dividends.csv                           Reference parsed dividends
├── interest.csv                            Reference parsed interest
├── gains.csv                               Reference computed gains
└── tax_summary_2023.xlsx                   Reference report output
```

**Note:** Test data is kept intentionally flat to avoid complexity. Integration tests reference these files directly without mirroring FARoot structure.

### Layer Definitions & Detailed Specifications

| Layer | Name | Location | Content | Mutable? | Source | Purpose |
|-------|------|----------|---------|----------|--------|---------|
| **1** | **Input** | `Input/vested.xlsx` | Brokerage export | ❌ No | User download | Tax period data |
| **2** | **Data** | `Data/Tickers/` + `Data/Reference/` | Prices, FX rates | ❌ No | Yahoo, SBI | Historical facts |
| **3** | **Output** | `Output/*` | All results | ✅ Yes | System | Computation results |

### Layer 1: Input (Immutable - User Provided)

**Location:** `~/Downloads/FACompute/Input/`

**Input/Brokerage/**
- **`vested.xlsx`** - DriveWealth brokerage export
  - Contains all transaction records: trades, dividends, interest, fees
  - User downloads directly from broker
  - System never modifies this file
  - Source of truth for tax period data

**Input/Parsed/**
- **`trades.csv`** - Parsed trade records (buy/sell transactions)
  - Generated by `tax vested parse` command
  - Extracted from current year's vested.xlsx
  - Contains quantity, dates, and prices for FIFO processing

- **`dividends.csv`** - Parsed dividend records
  - Generated by `tax vested parse` command
  - Extracted from vested.xlsx
  - Contains dividend amounts, dates, and tickers

- **`interest.csv`** - Parsed interest records
  - Generated by `tax vested parse` command
  - Extracted from vested.xlsx
  - Contains interest amounts and dates

- **`accounts_YYYY.csv`** - Prior year-end portfolio snapshot
  - Year-end balance from previous year's computation (e.g., accounts_2023.csv for 2024 computation)
  - Placed in Input/Parsed/ before computation
  - Serves as starting portfolio for current year's computation
  - Never generated during current computation cycle

### Layer 2: Data (Immutable - Once Downloaded)

**Location:** `~/Downloads/FACompute/Data/`

- **`Data/Tickers/*.json`** - Historical daily stock prices
  - Downloaded from Yahoo Finance on first request
  - Cached locally to avoid repeated API calls
  - Historical prices never change (price on 2023-12-31 is always the same)
  - Tickers include: AAPL, MSFT, VWO, IEF, NVDA

- **`Data/Reference/sbi_rates.csv`** - USD-INR exchange rates
  - SBI Telegraphic Transfer (TT) Buy rates
  - Downloaded/updated periodically from SBI
  - Used for currency conversions
  - Historical rates never change

### Layer 3: Output (Mutable - Generated at Runtime)

**Location:** `~/Downloads/FACompute/Output/` (exists ONLY at runtime, NOT in testdata)

#### Output/Computed/ (from `tax compute`)
- **`gains.csv`** - Capital gains computed using FIFO algorithm against trades
  - Calculated from Input/Parsed/trades.csv
  - Represents realized capital gains/losses

#### Output/YearEndBalance/ (Year-end snapshots)
- **`accounts_YYYY.csv`** - Portfolio snapshot at December 31st, YYYY
  - Computed during tax computation
  - Represents all holdings and their valuations at year-end
  - **Becomes Layer 1 input (Input/Parsed/) for next year's computation**
  - Example: `accounts_2024.csv` (generated this year) → copied to `Input/Parsed/accounts_2024.csv` before computing 2025

#### Output/Reports/ (Final ITR reports)
- **`tax_summary_YYYY.xlsx`** - Final tax report
  - Contains all INR-converted values
  - Organized by income type and asset
  - Ready for ITR filing (Schedule FA, FSI, etc.)
  - Includes valuations, gains, dividends, interest for the tax year

### Data Flow: How Layers Connect

```
Input/Brokerage/vested.xlsx (Layer 1: User Input)
    ↓
[tax vested parse]
    ↓
Input/Parsed/                          (Layer 1: Parsed Results)
├── trades.csv
├── dividends.csv
├── interest.csv
└── accounts_2023.csv (prior year)
    ↓
    + Data/Tickers/*.json (Layer 2)
    + Data/Reference/sbi_rates.csv (Layer 2)
    ↓
[tax compute]
    ↓
Output/                                (Layer 3: Generated Results)
├── Computed/
│   └── gains.csv
├── YearEndBalance/
│   └── accounts_2024.csv ← copy to Input/Parsed/ for 2025 computation
└── Reports/
    └── tax_summary_2024.xlsx ← ready for ITR filing
```

### Bootstrap Scenario: First Year (2022)

For the initial year with no prior portfolio:

1. User places `vested_2022.xlsx` in `Input/Brokerage/`
2. Run `tax vested parse` → generates `Input/Parsed/trades.csv`, `dividends.csv`, `interest.csv`
3. System starts with empty portfolio (no `accounts_2021.csv` in Input/Parsed/)
4. Run `tax compute` → generates `Output/YearEndBalance/accounts_2022.csv`
5. This `accounts_2022.csv` becomes the starting point for 2023 computation

### Year-to-Year Continuation

For subsequent years (e.g., 2024):

1. Copy prior year's `accounts_2023.csv` from `Output/YearEndBalance/` → `Input/Parsed/accounts_2023.csv`
2. Place new `vested_2024.xlsx` in `Input/Brokerage/`
3. Run `tax vested parse` → generates/overwrites `Input/Parsed/trades.csv`, `dividends.csv`, `interest.csv`
4. Run `tax compute` → uses `Input/Parsed/` files + Layer 2 data → generates `Output/YearEndBalance/accounts_2024.csv`
5. Copy `accounts_2024.csv` to `Input/Parsed/` for 2025 computation
6. Cycle repeats annually

### Why Three Layers?

- **Data Integrity:** Immutable Input and Data prevent accidental corruption
- **Reproducibility:** Same inputs always produce identical outputs
- **Auditability:** Clear separation shows what changed, when, and why
- **Correctness:** Output never contaminates Input during computation
- **Performance:** Cached external data (tickers, FX rates) reduces API calls
