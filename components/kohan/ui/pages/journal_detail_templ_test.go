...
var _ = Describe("Journal Detail Page Tests", func() {
	...
	Context("Sidebar Actions", func() {
		It("should render review and note actions", func() {
			...
			Expect(html).To(ContainSubstring(`x-on:click="sidebar.takenTag.submit(tag.text)"`))
			Expect(html).To(ContainSubstring(`x-bind:disabled="sidebar.takenTag.submitting || sidebar.takenTag.hasTag(tag.text)"`))
			Expect(html).To(ContainSubstring(`x-bind:class="tag.class"`))
