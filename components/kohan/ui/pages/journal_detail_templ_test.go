...
var _ = Describe("Journal Detail Page Tests", func() {
	...
	Context("Sidebar Actions", func() {
		It("should render review and note actions", func() {
			...
			Expect(html).To(ContainSubstring(`x-show="sidebar.mgmntTakenTags.showTakenTags()"`))
			Expect(html).To(ContainSubstring(`x-for="preset in sidebar.mgmntTakenTags.presets"`))
			Expect(html).To(ContainSubstring(`x-on:click="sidebar.mgmntTakenTags.submit(preset.text)"`))
			Expect(html).To(ContainSubstring(`x-bind:disabled="sidebar.mgmntTakenTags.submitting || sidebar.mgmntTakenTags.hasTag(preset.text)"`))
			Expect(html).To(ContainSubstring(`x-bind:class="preset.class"`))
