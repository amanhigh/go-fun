...
var _ = Describe("Journal Detail Page Tests", func() {
	...
	Context("Sidebar Actions", func() {
		It("should render review and note actions", func() {
			...
			Expect(html).To(ContainSubstring(`sidebar.reviewActions.submitter.hasError()`))
			Expect(html).To(ContainSubstring(`sidebar.reviewActions.submitter.messageClass`))
			Expect(html).To(ContainSubstring(`sidebar.reviewActions.submitter.error`))
			...
			Expect(html).To(ContainSubstring(`header.submitter.messageClass`))
			Expect(html).To(ContainSubstring(`header.submitter.error`))
			...
			Expect(html).To(ContainSubstring(`sidebar.reasonTagForm.submitter.messageClass`))
			Expect(html).To(ContainSubstring(`sidebar.reasonTagForm.submitter.error`))
			...
			Expect(html).To(ContainSubstring(`sidebar.noteForm.submitter.messageClass`))
			Expect(html).To(ContainSubstring(`sidebar.noteForm.submitter.error`))
			...
			Expect(html).To(ContainSubstring(`sidebar.takenTag.submitter.messageClass`))
			Expect(html).To(ContainSubstring(`sidebar.takenTag.submitter.error`))
	...
	Context("Loader State Bindings", func() {
		...
		It("should gate loaded content behind loader states", func() {
			Expect(html).To(ContainSubstring(`x-show="current.journal && !current.loader.isBusy() && !current.loader.hasError()"`))
		})
	})
})
